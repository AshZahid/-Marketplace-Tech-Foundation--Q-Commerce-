...
import { ourchef } from "@/sanity/lib/queries";
...
const allchef = await sanityfetch({query: ourchef})
      ...
      <h1 className="text-3xl font-bold text-center mb-10">Meet Our Chefs</h1>
       ...
       {/* APi data */}
           <div className="grid grid-cols-1 lg:grid-cols-3">
              {
                  allchef.map((items:any) =>(
                   <div key={items.id} className="shadow-lg">
                    <div className="relative">
                     <Image src={items.imageUrl} alt={items.name} width={400} height={200}/>                     
                    </div>
                        <h2 className="text-xl font-semibold text-center">{items.name}</h2>
                           <h3 className="text-sm text-center text-gray-500">{items.position}</h3>
                         </div>        
                              ))
                          }
                          </div>     
